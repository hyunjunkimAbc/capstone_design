...
data class Alarmdata( val imageUrl:String, val isRead:Boolean,val message:String, val timestamp: String,val username:String )
...
class AlarmViewModel: ViewModel() {
    ...
    fun loadalarm(){
        ...
        viewModelScope.launch {
            val alarminfo = db.collection("userAlarm").whereArrayContains("UserUid", "${Firebase.auth.uid}").get().await()
            for(data2 in alarminfo){
                ...
                val name=data2["username"] ?:""
               val item=Alarmdata(image, isread as Boolean, msg as String, time as String, name as String)
