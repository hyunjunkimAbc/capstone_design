...
class SelectHobbyActivity:AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(userinfo?.interest_array!=null) {
            ...
            R.id.toolbar_next_button->{
                if(key=="change") {
                    ...
                    db.collection("user").document(Firebase.auth.currentUser?.uid.toString())
                        ...
                        Log.d(
                            ContentValues.TAG,
                            SingleTonData.userInfo?.interest_array!!.toString()
                        )
    ...
    override fun onDestroy() {
        Log.d(ContentValues.TAG, "관심사 액티비티 종료문: "+SingleTonData.userInfo?.interest_array!!.toString())
        super.onDestroy()
    }
}