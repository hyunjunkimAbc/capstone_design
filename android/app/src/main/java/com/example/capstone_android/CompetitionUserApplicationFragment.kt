...
class CompetitionUserApplicationFragment : Fragment(){
    ...
    private var userNickname : TextView? = null// 유저 닉네임
    private var userBirth : TextView? = null // 유저 생년월일
    private var userLocation : TextView? = null// 유저 위치
    private var userEmail : TextView? = null // 유저 이메일

    private var competitionTitle : TextView? = null // 대회명
    private var competitionTime : TextView? = null // 대회 시작시간
    private var competitionLocation : TextView? = null // 대회 장소
    private var competitionWarnning :TextView? =null
    ...
    private data class Info( // 정보
        ...
        var start_time: String = "시작시간", // 대회 시작시간
        var end_time: String = "종료시간", // 대회 종료시간
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        userNickname  = binding.competitionUserNickName // 유저 닉네임
        userBirth  = binding.competitonUserBirth // 유저 생년월일
        userLocation  = binding.competitionUserLocation // 유저 위치
        userEmail  = binding.competitionUserEmail // 유저 이메일

        competitionTitle  = binding.competitionTitle // 대회명
        competitionTime  = binding.competitionTime // 대회 시작시간
        competitionLocation  = binding.competitionLocation // 대회 장소
        competitionWarnning = binding.competitionWarningInfo
        ...
        binding.buttonApply.setOnClickListener(){
            if(current_num < max_num) { // 최대 인원보다 적으면 신청 가능
                ...
                // 뒤로 보내기
                // 실패 토스트 출력
                findNavController().navigate(R.id.action_competitionUserApplicationFragment_to_meetingRoomInfoFragment)
            }
        }
            ...
            .addOnSuccessListener { document ->
                if (document != null) {
                    ...
                    applyInfo?.let {
                        ...
                        competitionTime?.setText(it.start_time + " ~ " + it.end_time)
                        competitionLocation?.setText(it.location)
                        competitionWarnning?.setText(it.info_text)
